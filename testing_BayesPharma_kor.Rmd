...
```{r}
kor_priors <- dr_priors(inhibitor = TRUE, top = 100)
kor_priors
```
...
```{r}
kor_inits <- dr_inits(inhibitor = TRUE)
kor_inits
```
...
```{r}
prior_densities(kor_sample_priors, half_max_label = "ic50")
```

RUN BAYESIAN MODEL

```{r}
kor_model <- dr_model(kor_antag,
                            kor_model1,
...
```{r}
BayesPharma::traceplot(kor_model, half_max_label = "ic50") 
```
...
```{r}
prior_posterior_densities(kor_model, half_max_label = "ic50")
```

POSTERIOR PARAMETER DENSITY DISTRIBUTIONS

```{r}
posterior_densities(kor_model, half_max_label = "ic50")
```
...
```{r}
prior_densities(kor_sample_priors, "predictors", "ic50")
```
...
```{r}
posterior_densities(kor_model2, half_max_label = "ic50")
```

SAMPLE OF DOSE RESPONSE CURVES FROM THE POSTERIOR DISTRIBUTION

```{r}
BayesPharma::posterior_response_draws(kor_model2)
```
...
---
```{r}
kor_antag <- change_col_names(kor_antag, "normalized_measurement", "log_dose", "Drug")
kor_antag
```

```{r}
stanvar_kor_model <- dr_stanvar_model(kor_antag,
                                      formula = dr_formula(TRUE),
                                      priors = kor_priors2,
                                      inits = kor_inits)
stanvar_kor_model
```
