...
xl_damgo_sigmoid <- "C:/Users/marti/Downloads/damgo_dr_reformatted.xlsx"

damgo <- readxl::read_excel(xl_damgo_sigmoid)
...
```{r}
damgo <- BayesPharma::calculate_log_dose(
  data = damgo,
  dose_col = dose_nM, 
  molar_concentration = 10^(-9))
damgo <- BayesPharma::change_col_names(
  data = damgo, 
...
```{r}
damgo_priors <- BayesPharma::sigmoid_prior(inhibitor = TRUE, 
                                       top = 100)
damgo_priors
```
...
```{r}
damgo_init <- BayesPharma::sigmoid_init(inhibitor = TRUE)
damgo_init
```
...
```{r}
sampling_priors <- BayesPharma::sigmoid_model(
  data = damgo,
  model_name = damgo1, 
...
BayesPharma::density_distributions(
  sampling_priors, half_max_label = "ic50")
```
...
## Running the Bayesian Sigmoid Model
sigmoid_model uses `brms` and `rstan` or `cmdstanr`

```{r}

damgo_model <- BayesPharma::sigmoid_model(
  data = damgo,
  model_name = damgo1,
  formula = BayesPharma::sigmoid_formula(),
  priors = damgo_prior,
...
```{r}
damgo_prior2 <- BayesPharma::sigmoid_priors(
  ec50 = brms::prior(normal(-6, 1.5), nlpar = "ec50"),
  hill = brms::prior(normal(-0.5, 0.5), nlpar = "hill"),
  top = 100,
  bottom = brms::prior(normal(12.5, 6.25), nlpar = "bottom")
                                       )
damgo_prior2
```
...
```{r}
sample_priors2 <- BayesPharma::sigmoid_model(
  damgo, 
  damgo2,
  formula = BayesPharma::sigmoid_formula(),
  prior = damgo_prior2,
...
```{r}
damgo_model2 <- BayesPharma::sigmoid_model(
  damgo, 
  damgo2,
