...
if (!is.null(model_evaluation_criteria)) { \# evalate fits model \<- model \|\> purrr::imap(function(model, i) { group_index \<- grouped_data[i, ] \|\> dplyr::select(-data) group_index_label \<- paste0( names(group_index), ":", group_index, collapse = ",") cat("Evaluating model fit for", group_index_label, "...\n", sep = "") model \<- model \|\> brms::add_criterion( criterion = model_evaluation_criteria, model_name = paste0("MuSyC:", group_index_label), reloo = TRUE) model }) } grouped_data \|\> dplyr::mutate( model = model) }
