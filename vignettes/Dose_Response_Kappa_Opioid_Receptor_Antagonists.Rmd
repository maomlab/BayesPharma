...
kor_sample_prior |>
  bayesplot::mcmc_dens(
    pars = tidyselect::starts_with("b_"))
```
...
drc_model <- kor_antag |>
  ...
  dplyr::group_modify( ~ {
    tibble::tibble(
      model = BayesPharma::drc_model(
        data = .x,
        fct = drc::L.4(
          fixed = c(NA, NA, 100, NA),
...
drc_model |>
  dplyr::group_by(Drug) |>
  dplyr::group_modify( ~ broom::tidy(.x$model[[1]])) |>
  dplyr::ungroup()
```
...
kor_post_means <- kor_model2 |>
  tidybayes::add_epred_draws(
    newdata = tidyr::expand_grid(
      log_dose = seq(from = -12, to = -4, length.out = 50),
      Drug = kor_model2$data[["Drug"]] |> unique()),
    value = "response",
    re_formula = NA) |>
  dplyr::group_by(log_dose, Drug) |>
  dplyr::summarize(
...
BTRX_335140_drc_plot <- plot_drc(
  ...
  ggplot2::geom_line(
    data = kor_post_means |>
      dplyr::filter(Drug |> stringr::str_detect("BTRX_335")),
    mapping = ggplot2::aes(
      x = log_dose, 
      y = response,
      color = "Bayes"),
...
BTRX_395750_drc_plot <- plot_drc(
  ...
  ggplot2::geom_line(
    data = kor_post_means |>
      dplyr::filter(Drug |> stringr::str_detect("BTRX_395")),
    mapping = ggplot2::aes(
      x = log_dose, 
      y = response,
      color = "Bayes"),
...
JNJ_drc_plot <- plot_drc(
  ...
  ggplot2::geom_line(
    data = kor_post_means |>
      dplyr::filter(Drug |> stringr::str_detect("JNJ")),
    mapping = ggplot2::aes(
      x = log_dose, 
      y = response,
      color = "Bayes"),
...
PF_drc_plot <- plot_drc(
  ...
  ggplot2::geom_line(
    data = kor_post_means |>
      dplyr::filter(Drug |> stringr::str_detect("PF")),
    mapping = ggplot2::aes(
      x = log_dose,
      y = response,
      color = "Bayes"),
...
drc_stats <- function(drc_fit, drug) {
  ...
  data.frame(.variable = c("hill", "bottom", "ic50")) |>
    dplyr::mutate(
      model = "drc",
      drug_label = drug,
...
```{r, echo=FALSE}
BTRX_335140_drc_stats[3,][BTRX_335140_drc_stats[3,]$.variable == "ic50", ".variable"] <- "ic50_BTRX_335140"
BTRX_395750_drc_stats[3,][BTRX_395750_drc_stats[3,]$.variable == "ic50", ".variable"] <- "ic50_BTRX_395750"
JNJ_drc_stats[3,][JNJ_drc_stats[3,]$.variable == "ic50", ".variable"] <- "ic50_JNJ"
PF_drc_stats[3,][PF_drc_stats[3,]$.variable == "ic50", ".variable"] <- "ic50_PF"

drc_ic50 <- dplyr::bind_rows(
  BTRX_335140_drc_stats[3,], 
  BTRX_395750_drc_stats[3,],
  JNJ_drc_stats[3,],
  PF_drc_stats[3,])

BTRX_335140_drc_stats[1,][BTRX_335140_drc_stats[1,]$.variable == "hill", ".variable"] <- "hill_BTRX_335140"
BTRX_395750_drc_stats[1,][BTRX_395750_drc_stats[1,]$.variable == "hill", ".variable"] <- "hill_BTRX_395750"
JNJ_drc_stats[1,][JNJ_drc_stats[1,]$.variable == "hill", ".variable"] <- "hill_JNJ"
PF_drc_stats[1,][PF_drc_stats[1,]$.variable == "hill", ".variable"] <- "hill_PF"

drc_hill <- dplyr::bind_rows(
  BTRX_335140_drc_stats[1,], 
  BTRX_395750_drc_stats[1,],
  JNJ_drc_stats[1,],
  PF_drc_stats[1,])

BTRX_335140_drc_stats[2,][BTRX_335140_drc_stats[2,]$.variable == "bottom", ".variable"] <- "bottom_BTRX_335140"
BTRX_395750_drc_stats[2,][BTRX_395750_drc_stats[2,]$.variable == "bottom", ".variable"] <- "bottom_BTRX_395750"
JNJ_drc_stats[2,][JNJ_drc_stats[2,]$.variable == "bottom", ".variable"] <- "bottom_JNJ"
PF_drc_stats[2,][PF_drc_stats[2,]$.variable == "bottom", ".variable"] <- "bottom_PF"

drc_bottom <- dplyr::bind_rows(
  BTRX_335140_drc_stats[2,], 
...
```{r bayes-vs-drc, echo=FALSE}
bayes_vs_drc <- function(
    model,
    filter_label, 
    drc) {
  ...
  posterior <- model |>
    ...
    dplyr::mutate(.variable =
        .variable |> stringr::str_replace_all("b_|_Intercept", ""))
  
  model_type <- c("BayesPharma" = "blue", "drc" = "red")
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::labs(
      ...
      y = "Drug",
      color = "Model") +
    ggplot2::scale_color_manual(values = model_type)
}
```


```{r model2-bayes-vs-drc}
#| dependson=c("model2", "bayes-vs-drc")
