...
if(Sys.info()["machine"] == "arm64"){
  cat("rstan does not currently support apple M1 chips, 
      setting stan_backend = 'cmdstanr'")
...
values to be labelled "response" and the dose to be in log scale; therefore, 
it may be necessary to use the `calculate_log_dose` and `change_col_names` 
functions before running the model. 
The loaded data already has a log dose column, so for this data set we don't 
need to use the `calculate_log_dose` function.
...
The prior distributions cover a broad plausible range of values.
...
run. Therefore, we are in good shape to move forward with the analysis. 
...
for BTRX-395750 and JNJ fits the structure of the data pretty well, the model 
...
parameters (prior is pink and posterior is teal). this can be useful if you are
interested in comparing the initial prior with the posterior distribution 
...
```{r}
BayesPharma::posterior_densities(kor_model, half_max_label = "ic50")
```

Displayed below is a plot of a sample of 100 sigmoid dose-response curves from 
the posterior distribution (purple) and the mean curve (black): 
...
BayesPharma::plot_draws_data(model = kor_model,
                             ...
                             predictor_name = "Drug",
                             data = kor_antag,
                             measurement = kor_antag$response,
                             title = NULL,
                             xlabel = "Log[Molar]",
...
```{r}
BayesPharma::traceplot(kor_model2, "predictors", "ic50") 
```
...
```{r}
kor_model <- add_loo_criterion(kor_model, "kor_model1")
kor_model2 <- add_loo_criterion(kor_model2, "kor_model2")
...
#rbind(BTRX_335140_drc_stats, BTRX_395750_drc_stats, JNJ_drc_stats, PF_drc_stats)
```

```{r}

basic_stats(kor_model2, predictors_col_name = "Drug", half_max_label = "ic50")
...
BTRX_335140_bayes_stats <-
  basic_stats(kor_model2, predictors_col_name = "Drug", 
              half_max_label = "ic50")[grepl("BTRX_335140", 
                                basic_stats(kor_model2, 
                                            predictors_col_name = "Drug", 
                                            half_max_label = "ic50")$variables),] %>%
...
BTRX_395750_bayes_stats <-
  basic_stats(kor_model2, predictors_col_name = "Drug", 
              half_max_label = "ic50")[grepl("BTRX_395750", 
                                basic_stats(kor_model2)$variables),]%>%
  dplyr::mutate(variables = stringr::str_extract(variables,
                                                 "[a-zA-Z0-9]+.{1,100}") %>%
...
JNJ_bayes_stats <-
  basic_stats(kor_model2, predictors_col_name = "Drug", 
              half_max_label = "ic50")[grepl("JNJ", basic_stats(kor_model2)$variables),] %>%
  dplyr::mutate(variables = stringr::str_extract(variables,
                                                 "[a-zA-Z0-9]+.{1,100}") %>%
...
PF_bayes_stats <-
  basic_stats(kor_model2, predictors_col_name = "Drug", 
              half_max_label = "ic50")[grepl("PF", basic_stats(kor_model2)$variables),] %>%
  dplyr::mutate(variables = stringr::str_extract(variables,
                                                 "[a-zA-Z0-9]+.{1,100}") %>%
...
BTRX_335140_interval <- dplyr::bind_rows(BTRX_335140_drc_stats, 
                                         BTRX_335140_bayes_stats[1:3,]) %>%
  ggplot2::ggplot(
    ggplot2::aes(y = variables, x = mean, xmin = l_ci, 
                 xmax = u_ci,color = model)) +
  ggdist::geom_pointinterval(position = ggplot2::position_dodge(width = .3)) +
  ggplot2::scale_color_manual(values = model) +
  ggplot2::labs(title = "BTRX_335140")

BTRX_39575_interval <- dplyr::bind_rows(BTRX_395750_drc_stats, 
                                        BTRX_395750_bayes_stats[1:3,]) %>%
  ggplot2::ggplot(ggplot2::aes(y = variables, x = mean, xmin = l_ci, 
                               xmax = u_ci, color = model)) +
  ggdist::geom_pointinterval(position = ggplot2::position_dodge(width = .3)) +
  ggplot2::scale_color_manual(values = model) +
  ggplot2::labs(title = "BTRX_395750")

JNJ_interval <- dplyr::bind_rows(JNJ_drc_stats, JNJ_bayes_stats[1:3,]) %>%
  ggplot2::ggplot(ggplot2::aes(y = variables, x = mean, xmin = l_ci, 
                               xmax = u_ci, color = model)) +
  ggdist::geom_pointinterval(position = ggplot2::position_dodge(width = .3)) +
  ggplot2::scale_color_manual(values = model) +
...
PF_interval <- dplyr::bind_rows(PF_drc_stats, PF_bayes_stats[1:3,]) %>%
  ggplot2::ggplot(ggplot2::aes(y = variables, x = mean, xmin = l_ci, 
                               xmax = u_ci, color = model)) +
  ggdist::geom_pointinterval(position = ggplot2::position_dodge(width = .3)) +
  ggplot2::scale_color_manual(values = model) +
...
than the standard errors provided by the drc method. 
