...
values to be labeled "response", the column containing perturbation labels 
(Drug, mutant, etc.) to be labeled predictors, and the dose to be in log scale; 
...
the posterior distribution (purple) and and the median quantile intervals: 

```{r}
posterior_draws_plot(model = kor_model, data = kor_antag, 
                     predictors_col_name = "Drug",
                     lower = -12, upper = -4, n = 50, 
                     facet_var = Drug, title = "Dose-Response Posterior Draws",
...
```{r}
kor_model <- BayesPharma::add_loo_criterion(kor_model, "kor_model1")
kor_model2 <- BayesPharma::add_loo_criterion(kor_model2, "kor_model2")
...
```{r}
BTRX_335140_df <- kor_antag[grepl("BTRX_335140", kor_antag$predictors),]
BTRX_395750_df <- kor_antag[grepl("BTRX_395750", kor_antag$predictors),]
JNJ_df <- kor_antag[grepl("JNJ", kor_antag$predictors),]
PF_df <- kor_antag[grepl("PF", kor_antag$predictors),]
```
...
```
```{r}
bayes_vs_drc(kor_model2, "ec50", "predictors", "ic50", drc_ic50) 
```

(The drc results for the hill are displayed as a positive value although the 
curve has a negative slope.)
```{r}
hill_bd1 <- bayes_vs_drc(kor_model2, "hill", "predictors", "hill", drc_hill) 
hill_bd2 <- bayes_vs_drc(kor_model2, "hill", "predictors", "hill", drc_hill) +
  ggplot2::xlim(-6, 14)
...
```{r}
bayes_vs_drc(kor_model2, "bottom", "predictors", "bottom", drc_bottom) 
```
