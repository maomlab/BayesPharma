...
kor_prior <- BayesPharma::sigmoid_prior(
  inhibitor = TRUE,
  top = 100)
kor_prior
```
...
Monte Carlo Marko Chains of each parameter. The default initial values are:
`ec50 = -9`, `hill = -1`, `top = 100`, and `bottom = 0`.

```{r default-init}
#| warning=FALSE,
#| message=FALSE
kor_init <- BayesPharma::sigmoid_init()
```
...
an argument to the `sigmoid_model` function.
...
`sigmoid_formula` to be *predictors = 0 + Drug*. 
...
kor_model <- BayesPharma::sigmoid_model(
  data = kor_antag,
  formula = BayesPharma::sigmoid_formula(0 + Drug), 
  prior = kor_prior,
  init = kor_init)
```
...
```{r model-prior2}
kor_prior2 <- BayesPharma::sigmoid_prior(
