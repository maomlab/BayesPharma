...
kor_sample_prior |>
  bayesplot::mcmc_dens(
    pars = dplyr::vars(
      ...
      b_top_Intercept,
      b_bottom_Intercept)) +
  ggplot2::ggtitle("Prior Density Distributions") +
  ggplot2::facet_grid(
    ~ Parameter,
    labeller = ~ .|> stringr::str_replace_all("b_|_Intercept", ""))
```
...
```{r model-prior2}
kor_prior2 <- BayesPharma::sigmoid_antagoinst_prior(
...
kor_sample_prior <- BayesPharma::sigmoid_antagoinst_model(
  kor_antag,
  prior = kor_prior2,
  init = kor_init,
  sample_prior = "only")
...
kor_model2 <- BayesPharma::sigmoid_antagoinst_model(
  kor_antag,
  formula = BayesPharma::sigmoid_antagonist_formula(0 + Drug), 
  prior = kor_prior2,
  init = kor_init,
