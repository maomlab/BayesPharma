...
```{r snugdock-load-data, dependson=c("load-packages")}
...
```{r stugdock-fit-model, output=FALSE, dependson=c("load-packages", "stugdock-load-data")}
model <- data %>%
...
```{r mode-fit, dependson=c("load-packages", "stugdock-fit-model")}
model
```
...
```{r plot-data-model-rmsd, dependson=c("load-packages", "stugdock-fit-model", "snugdock-collect-draws")}
ggplot2::ggplot(data = data) + 
  ggplot2::theme_bw() +
  GeomIndicator::geom_indicator(
    data = pnear_scores,
