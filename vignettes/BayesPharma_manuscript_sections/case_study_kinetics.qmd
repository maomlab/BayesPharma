...
\scriptsize
```{r tQ-simulate-data-single, echo = TRUE}
...
```{r tQ-model-single}
#| echo = TRUE,
#| dependson = c("tQ-simulate-data-single", "stanvars-ode-tQ", "stanvars-tQ-single")
...
#| dependson = c("tQ-model-single"),
#| echo=TRUE
model_single_prior <- model_single |> update(sample_prior = "only")
```
...
model_multiple_prior <- model_multiple |> brms:::update(sample_prior = "only")
```
