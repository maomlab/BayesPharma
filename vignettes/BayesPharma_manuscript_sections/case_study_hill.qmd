...
```{r kor-priors}
#| echo=TRUE
kor_prior <- BayesPharma::sigmoid_prior(inhibitor = TRUE, top = 100)
kor_prior
```
\normalsize
...
kor_sample_priors <- BayesPharma::sigmoid_model(
...
kor_model <- BayesPharma::sigmoid_model(
  data = kor_antag |> dplyr::select(substance_id, log_dose, response),
  formula = BayesPharma::sigmoid_formula(predictors = 0 + substance_id), 
  priors = kor_priors,
...
\scriptsize
```{r kor-model-priors2}
kor_priors2 <- BayesPharma::sigmoid_prior(
...
kor_model2 <- BayesPharma::sigmoid_model(
  data = kor_antag |> dplyr::select(substance_id, log_dose, response),
  formula = BayesPharma::sigmoid_formula(0 + substance_id), 
  priors = kor_priors2,
  init = BayesPharma::sigmoid_init())
