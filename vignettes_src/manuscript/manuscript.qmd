...
abstract: |
  ...
  is an `R` package built around the `Stan` ecosystem that facilitates a
  principled Bayesian workflow. It can be used to fit and analyze
  foundational pharmacology models, serve as a pedagogical framework for
  learning Bayesian methods, and help build and analyze sophisticated
  pharmacological models. We describe the `BayesPharma` framework here, and
  illustrate its application through several case studies.  
