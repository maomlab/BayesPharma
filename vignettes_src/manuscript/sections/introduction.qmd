...
Together, these stages of model fitting and evaluation are the foundation for a principled work, that is outlined in more detail in [@Gelman2020-ab]. Understanding what steps need to be communicated depends on the the context, the case-studies in the following sections illustrate templates that can be for typical Pharmacology analyses.
