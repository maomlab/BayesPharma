...
\code{sample_prior = "only"} as  an argument to the \code{sigmoid_antagonist_model}
...
#| fig.width = 9,
#| fig.height = 3,
#| fig.cap="KOR antagonists conditional effects. The blue lines are samples from the \texttt{BayesPharma} kor_model posterior distribution, the orange line is the conditional mean, and the purple line is the conditional mean for the \texttt{drc} model fit."
